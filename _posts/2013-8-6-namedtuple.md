...
interesting. If you haven't heard of namedtuple before, 
[check it out](http://pymotw.com/2/collections/namedtuple.html); 
it's a very useful builtin.
...
underscore 'em for a new (and hopefully valid) name.
...
Now we check for duplicate fields.
...
tuple str representation for `argtxt`, and the notation for duplicating a
...
with it? Execute it in a temporary namespace, of course. Check out the use of
`exec ... in`. 
...
For me, the most interesting part of the above implementation was the actual
execution of the code string into a namespace that existed solely for the 
purpose of that one evaluation. It emphasized the simplicity of Python in
my mind: all namespaces, including modules and classes, really just reduce
to dicts.
